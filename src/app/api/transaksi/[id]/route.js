...
export async function PUT(request, { params }) {
    ...
    try {
        const { judul, nominal, tanggal, jam, catatan, bukti_url } = await request.json();
        ...
        const queryText = `
            UPDATE "Transaction" 
            SET judul = $1, nominal = $2, tanggal = $3, jam = $4, catatan = $5, bukti_url = $6, updated_at = $7
            WHERE id = $8 AND user_id = $9 -- Memastikan hanya pemilik yang bisa mengedit
            RETURNING *
        `;

        const values = [judul, parseFloat(nominal), transactionTimestamp, timeOnly, catatan, bukti_url, now, id, userId];
